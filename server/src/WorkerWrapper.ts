...
export class WorkerWrapper<Events extends Record<string, any>> {
  ...
  constructor(
    ...
    protected init: Events["init"],
    protected eventHandlers: {
      [
        K in keyof Omit<
          Events,
          keyof SongSourceEvents | keyof LifecycleEvents<any>
        >
      ]: (event: Events[K]) => void;
    },
  ) {
  ...
  start() {
    this.worker = new Worker(
      new URL(`./workers/${this.name}.ts`, urlBase).toString(),
      {
        type: "module",
        name: this.name,
        deno: {
    ...
    this.worker.addEventListener("error", async (ev) => {
      ...
      log.error(
        `Worker ${this.name} errored: ${ev.message} -- restarting in 30s`,
      );
