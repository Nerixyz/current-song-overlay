...
export class MessageHandler<EventMap extends Record<string, any>> {
  protected eventMap = new Map<
    keyof EventMap,
    ArrayOrSingle<EventEntry<EventMap[keyof EventMap]>>
  >();
  ...
  private insertEvent<K extends keyof EventMap>(
    key: K,
    eventHandler: EventEntry<EventMap[K]>,
  ): void {
    ...
    if (this.eventMap.has(key)) {
      const entry = this.eventMap.get(key) as ArrayOrSingle<
        EventEntry<EventMap[K]>
      >;
      if (Array.isArray(entry)) {
        ...
        this.eventMap.set(key, [
          entry as EventEntry<Values<EventMap>>,
          eventHandler as EventFn<Values<EventMap>>,
        ]);
      }
    } else {
