...
export function getModuleOptions<K extends keyof ModuleOptions>(
  name: K,
): ModuleOptions[K] {
  const mod = config.modules?.[name];
...
export function getVarOrDefault<K extends keyof Config>(
  key: K,
...
async function initConfig() {
  const hasConfig = await Deno.stat("config.json").then(() => true).catch(() =>
    false
  );
  if (!hasConfig) {
